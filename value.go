...
func toValue_reflectValuePanic(value interface{}, kind reflect.Kind) {
	...
	switch kind {
	case reflect.Struct:
		panic(newError(nil, "TypeError", 0, nil, "invalid value (struct): missing runtime: %v (%T)", value, value))
	case reflect.Map:
		panic(newError(nil, "TypeError", 0, nil, "invalid value (map): missing runtime: %v (%T)", value, value))
	case reflect.Slice:
		panic(newError(nil, "TypeError", 0, nil, "invalid value (slice): missing runtime: %v (%T)", value, value))
	}
}
